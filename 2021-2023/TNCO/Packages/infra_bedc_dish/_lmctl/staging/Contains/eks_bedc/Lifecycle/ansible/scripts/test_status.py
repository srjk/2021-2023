...
repo_name=data["custom_fields"]["eksCodeCommitRepo"]
deploy_env=data["custom_fields"]["deploy_env"]
order_id=data["custom_fields"]["lcm_orderId"]
pname="eks-pipeline-"+ deploy_env
pipeline_name=repo_name.replace("eks",pname)
response=''
id=''
...
putFileContent=[]
content= '' + json.dumps(data["custom_fields"]['eksConfig'],indent=2) +''
putFiles_dummy={
            'filePath': '',
            'fileMode': 'NORMAL',
            'fileContent': content
