...
f=open('/tmp/test_bedc')
data = json.load(f)
f.close()

profile=data["custom_fields"]["profile_name"]
repo_name=data["custom_fields"]["nfsCodeCommitRepo"]
...
putFileContent=[]
content= '' + json.dumps(data["custom_fields"]['nfsConfig'],indent=2) +''
putFiles_dummy={
            'filePath': '',
            'fileMode': 'NORMAL',
            'fileContent': content
...
while(sf == "InProgress"):
  ...
  while(sf1=='RUNNING'):                                             
    ...
    for i in response["events"]:                                              
      if "taskSubmittedEventDetails" in i.keys():                                
        sfname2=json.loads(i["taskSubmittedEventDetails"]['output'])['ExecutionArn']                                                                                      
        response=client.get_execution_history(executionArn=sfname2)                                    
        for j in response["events"]:                               
          if "taskSubmittedEventDetails" in j.keys():                            
            sfbuildarn=json.loads(j["taskSubmittedEventDetails"]['output'])['Build']["Arn"]
            if sfbuildarn not in dummy:                                                    
              ...
              sfs=cloudwatchlogpri(sfs,2,data,id,executionid,loggroupname)
              if(sfs!='SUCCEEDED'):
  ...
  if(len(response["stageStates"][2]) != 0): 
    if(response["stageStates"][2]["latestExecution"]["pipelineExecutionId"]==id):
      ##print(response)
      if(len(response["stageStates"][2]["actionStates"][0])!=0 and 'latestExecution' in response["stageStates"][2]["actionStates"][0] and 'externalExecutionId' in response["stageStates"][2]["actionStates"][0]["latestExecution"]):
        sf=response["stageStates"][2]["latestExecution"]["status"]    
if(sf!='Succeeded'):
