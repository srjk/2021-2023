...
putFiles_dummy["filePath"]=path_repo+'applicationi_config.json'
putFiles_dummy["fileContent"]=content
putFileContent.append(putFiles_dummy)
...
broker = 'ib-orc-strimzi-001-kafka-bootstrap.cyswy001n-ns-ib-orch-001:9092'
topic = 'nflcmLog'
...
code_build_client = session.client('codebuild')
...
#print(code_build_response)
cloudwatch_id=""
ids= code_build_response["ids"]
for i  in range(len(ids)):
   response = code_build_client.batch_get_builds(
       ids=[
        ids[i]
       ]
     )
#print("---------------------")
#print(response)
   #print(response["builds"][i]["resolvedSourceVersion"])
   if(response["builds"][i]["resolvedSourceVersion"] == commitId):
        cloudwatch_id=response["builds"][i]["id"]
        break;
#print(cloudwatch_id)

code_build_id=cloudwatch_id[cloudwatch_id.rfind(':'):].strip(':')
#print("new id ==", code_build_id)
